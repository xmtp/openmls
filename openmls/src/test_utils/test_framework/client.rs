...
impl<Provider: OpenMlsProvider> Client<Provider> {
    ...
    pub fn join_group(
        ...
        let new_group = staged_join.into_group(&mut self.provider)?;
        self.groups
            .write()
            .expect("An unexpected error occurred.")
    ...
    pub fn receive_messages_for_group<AS: Fn(&Credential) -> bool>(
        ...
        if sender_id == self.identity && message.content_type() == ContentType::Commit {
            group_state.merge_pending_commit(&mut self.provider)?
        } else {
            ...
            let processed_message = group_state
                .process_message(&mut self.provider, message.clone())
            ...
            match processed_message.into_content() {
                ...
                ProcessedMessageContent::StagedCommitMessage(staged_commit) => {
                    ...
                    group_state.merge_staged_commit(&mut self.provider, *staged_commit)?;
                }
            }
        }
    ...
    > {
        ...
        let (msg, welcome_option, group_info) = match action_type {
            ActionType::Commit => {
                let bundle =
                    group.self_update(&mut self.provider, &signer, LeafNodeParameters::default())?;
            ...
            ActionType::Proposal => {
                let (msg, _) =
                    group.propose_self_update(&mut self.provider, &signer, leaf_node_parameters)?;
        ...
        let action_results = match action_type {
            ActionType::Commit => {
                let (messages, welcome_message, group_info) =
                    group.add_members(&mut self.provider, &signer, key_packages)?;
                (
                    vec![messages],
                    Some(
                        welcome_message
            ...
            ActionType::Proposal => {
                ...
                for key_package in key_packages {
                    let message = group
                        .propose_add_member(&mut self.provider, &signer, key_package)
                        .map(|(out, _)| out)?;
        ...
        let action_results = match action_type {
            ActionType::Commit => {
                let (message, welcome_option, group_info) =
                    group.remove_members(&mut self.provider, &signer, targets)?;
                (
                    vec![message],
                    welcome_option.map(|w| w.into_welcome().expect("Unexpected message type.")),
                    group_info,
            ...
            ActionType::Proposal => {
                ...
                for target in targets {
                    let message = group
                        .propose_remove_member(&mut self.provider, &signer, *target)
                        .map(|(out, _)| out)?;
                    messages.push(message);
                }
                (messages, None, None)
