...
impl<Provider: OpenMlsProvider> MemberState<'_, Provider> {
    pub fn assert_group_storage_state_matches(&mut self, to_compare: GroupStorageState) {
        let state_now = self.group_storage_state();
        assert!(to_compare == state_now);
    }
    pub fn assert_non_proposal_group_storage_state_matches(&mut self, to_compare: GroupStorageState) {
        let state_now = self.group_storage_state();
