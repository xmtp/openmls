...
fn unknown_extensions(n: usize) -> Vec<Extension> {
    // Keep only types that round-trip as `Extension::Unknown`. This skips GREASE
    // values and any registered type in this range — notably the fork's
    // `ImmutableMetadata` (0xf000), which would otherwise deserialize as a real
    // (empty) metadata extension and fail.
    (0x0100u16..)
