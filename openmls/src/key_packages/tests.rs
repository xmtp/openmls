...
fn key_package_validation_with_many_extensions() {
    ...
    // registered types and the GREASE values. (Upstream used 0xf000, but the
    // fork registers `ImmutableMetadata` there, so this range starts lower.)
    const FIRST_TYPE: u16 = 0x8000;
