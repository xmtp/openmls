...
impl MlsGroup {
    ...
    pub async fn resolve_app_data_commit<Provider: OpenMlsProvider>(
        ...
        // Take-stage-set rather than the closure form of
        // `ProcessedMessage::resolve_app_data_commit`: staging touches storage,
        // which is `async` here and cannot be awaited inside that closure.
        let unresolved_commit = processed_message.take_unresolved_app_data_commit()?;
        let staged_commit = self
            .stage_app_data_commit(provider, unresolved_commit, app_data_dict_updates)
            .await?;
        processed_message.set_staged_commit(staged_commit);
