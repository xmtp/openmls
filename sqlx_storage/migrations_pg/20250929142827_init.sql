-- The PostgreSQL twin of `migrations/20250929142827_init.sql`. It differs from
-- the SQLite version in three ways:
--   * BLOB columns are BYTEA;
--   * `leaf_index` is BIGINT, because PostgreSQL has no unsigned types and the
--     provider binds the u32 leaf index as an i64;
--   * the CHECK constraint is named, so a later migration can replace it in
--     place instead of rewriting the table the way SQLite has to.
